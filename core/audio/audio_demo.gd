...
## and toggles them on a bar boundary, and a [ParameterFader] demo sweeping a
## filter cutoff and a reverb wet mix.
...
func _process(delta: float) -> void:
	_update_bar_beat_label()
	_advance_fader_sweep(delta)


func _update_bar_beat_label() -> void:
...
func _advance_fader_sweep(delta: float) -> void:
