...
## The most the background sample ever shifts, in UV. Kept small on purpose:
## the medium should be nearly invisible at rest and only read as a ripple
## where the fluid is actually moving. Zero turns the bend off entirely.
@export_range(0.0, 0.1, 0.001) var strength: float = 0.012

## Change in speed from one simulation cell to the next, in cells/second, at
## which the bend reaches half of [member strength]. Lower this to make a
## gentle drift visible; raise it so only a hard swish shows.
